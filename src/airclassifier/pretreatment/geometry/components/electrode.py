...
This is a refactored version of the original electrode.py placed in the
components directory. The electrode is an internal component of the oven
and does not have external ConnectionPorts.
...
Phase 3 additions (engineering guide sections 4.1.4, 4.1.5):
...
class ElectrodeParams:
    ...
    # Position (lower-left corner of combined plates)
...
class ElectrodeGeometry:
    ...
    The electrode is an internal oven component without external ports.

    Provides three correction fields (engineering guide sections 4.1.4, 4.1.5):
    ...
    def __init__(self, params: Optional[ElectrodeParams] = None):
        ...
        self._upper_vertices: Optional[np.ndarray] = None
        self._upper_triangles: Optional[np.ndarray] = None
        self._lower_vertices: Optional[np.ndarray] = None
        self._lower_triangles: Optional[np.ndarray] = None
    ...
    def generate_upper_mesh(
        ...
        verts, tris = box_mesh(0.0, y, 0.0, L, t, W)
    ...
    def generate_lower_mesh(self) -> Tuple[np.ndarray, np.ndarray, dict]:
        ...
        verts, tris = box_mesh(0.0, -t, 0.0, L, t, W)
        return verts, tris, {"type": "lower_electrode"}

    def generate_mesh(self) -> Tuple[np.ndarray, np.ndarray, dict]:
        """Generate combined electrode mesh (lower only, for compatibility).
    ...
    def get_perforation_correction_field(
        ...
        Engineering guide section 4.1.5::
    ...
    def get_fringe_field_correction(
        ...
        Uses the Palmer formula (engineering guide section 4.1.4)::
    ...
    def ports(self) -> Dict[str, "ConnectionPort"]:
        """Electrode is internal to the oven - no external ports."""
        return {}
