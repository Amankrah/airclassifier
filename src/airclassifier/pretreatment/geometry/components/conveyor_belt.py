...
class ConveyorBeltParams:
    ...
    # ── Structural frame ─────────────────────────────────────────────
    frame_length_m: float = 4.724       # [m]  Frame = bed length (472.4 cm measured)
...
class ConveyorBeltGeometry:
    ...
    def generate_bed_structure_mesh(self) -> Tuple[np.ndarray, np.ndarray, dict]:
        ...
        # ── 1. Longitudinal side rails (shortened at outfeed for bin) ──
        # Left rail (z ≈ 0)
        parts.append(box_mesh(0, -rs, 0, rail_end, rs, rs))
        # Right rail (z ≈ W - rs)
        ...
        # Shortened at outfeed to clear collection bin
        deck_thick = 0.003   # 3 mm sheet
        deck_z0 = p.belt_z0
        deck_z1 = p.belt_z1
        parts.append(box_mesh(
            nose, -deck_thick, deck_z0,
            rail_end - nose, deck_thick, deck_z1 - deck_z0,
        ))

        # NOTE: Attenuation ducts removed from conveyor — they are now
        # the infeed tunnel and outfeed tunnel components attached to
        # the oven chamber walls (see hopper.py and machine.py).
    ...
    def generate_wheels_mesh(self) -> Tuple[np.ndarray, np.ndarray, dict]:
        ...
        if n_carry > 0:
            ...
            carry_start = x_tail + 0.50
            carry_end = x_head - 0.50
