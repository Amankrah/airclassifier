...
## GPU use during calibration

When device is `"cuda"` and Warp kernel imports succeed (`coupling._GPU_KERNELS_OK`), the physics loop runs on GPU (advection, thermal, moisture, power density, properties). Otherwise it falls back to CPU (NumPy). The example and calibration both print `Physics: GPU (Warp)` or `Physics: CPU (NumPy)` so you can confirm which path is active.

Calibration runs the simulator once per optimizer evaluation (each with a full-duration run, e.g. 2794 s). The **simulation itself is currently CPU-bound**: advection, RF field (series-capacitor model), thermal FDM, moisture, and material-property updates use **NumPy** in this code path. Warp/CUDA is initialized (you will see “Warp 1.11.0”, “cuda:0” in the log), but the heavy per-step work runs on CPU. So **GPU in use when Physics: GPU (Warp) is printed; otherwise CPU (NumPy) fallback**. The optimizer (scipy’s differential_evolution) also runs on CPU between evaluations. To increase GPU use in the future, the calibration path would need to call the GPU-accelerated kernels (e.g. Warp thermal/moisture/RF) where available.
